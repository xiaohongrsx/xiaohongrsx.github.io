...
import { stageGiscusConfig } from "./giscus.mjs";

// 仅对站点级 assets/ 下的 CSS/JS 做 minify，避免压到 posts/pages 内的随附资源。
...
function stageAssetGroup(sourceRoot, sourceFiles, stagingSiteDir, targetPrefix, statusMap, label) {
	...
	for (const sourcePath of sourceFiles) {
		...
		if (shouldMinify) {
			...
			const { code: minifiedCode } = transformSync(sourceCode, {
...
export async function buildSite({ outDirName, jobs, configPath, cacheRoot }) {
	...
	// 6. 生成 Giscus 配置脚本
	// 7. 生成 RSS
	// 8. 对比旧 _site 标记 deleted，并将临时目录移动到目标输出目录
	...
	writeJson(paths.postsMetadataJsonPath, visiblePosts);

	const tagValues = visiblePosts.flatMap((post) => post.tags);
	const categoryValues = visiblePosts.map((post) => post.category).filter(Boolean);
	...
	// 6) 生成 Giscus 配置脚本（assets/core/giscus.config.js）。
	stageGiscusConfig(paths.siteConfigPath, outputSiteDir, paths.stagingSiteDir, statusMap);

	// 7) 生成 RSS（输出根目录 rss.xml）。
